...
Grid2D::Grid2D(float S1_min, float S1_max, int N1,
...
float Grid2D::interpolate(const std::vector<float>& V, float S1, float S2) const {
    ...
    // Bilinear interpolation
    float v00 = V[i0 * N2_ + j0];
    float v01 = V[i0 * N2_ + j1];
    float v10 = V[i1 * N2_ + j0];
    float v11 = V[i1 * N2_ + j1];
...
{
    float S1_min = 0.0;
    float S1_max = S1_0 * spaceFactor;
    float S2_min = 0.0;
    float S2_max = S2_0 * spaceFactor;
